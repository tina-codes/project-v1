...
from the database, while allowing track and artist data to remain.
...
when the user selects an item type (Tracks, Artists or Genres) from the main navitation, or a
timespan (month, six months, all time) from the view options menu. A useMemo hook prevents 
components from rendering unnecessarily. The page relies heavily on state logic to dictate 
what data appears where, and when, and also uses state to hold layered data and user photos.
...
timespans - short term (1 month), medium term (6 months) or long term (all time). The default 
view shown on login is top tracks, short term.
